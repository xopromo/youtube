...
for file_idx, html_file in enumerate(html_files):
    try:
        ...
        # Pattern: from_name div followed by text div
        ...
        for match in re.finditer(message_pattern, content, re.DOTALL):
            ...
            if msg_text:
                ...
                # Sample message
                if not participants[from_name]['sample_message'] and len(msg_text) > 5:
                    participants[from_name]['sample_message'] = msg_text[:100]

        if (file_idx + 1) % 50 == 0:
            print(f"  Processed {file_idx + 1}/{len(html_files)} files, {len(participants)} participants so far")
...
for name, info in sorted_participants:
    ...
    ws.cell(row=row_num, column=2).value = info['tg_username']
    ws.cell(row=row_num, column=3).value = info['tg_link']
    ws.cell(row=row_num, column=4).value = ', '.join(info['phone']) if info['phone'] else ''
    ws.cell(row=row_num, column=5).value = ', '.join(info['email']) if info['email'] else ''
    ws.cell(row=row_num, column=6).value = ', '.join(info['website']) if info['website'] else ''
    ws.cell(row=row_num, column=7).value = ', '.join(info['instagram']) if info['instagram'] else ''
    ws.cell(row=row_num, column=8).value = info['location']
    ws.cell(row=row_num, column=9).value = info['message_count']
    ws.cell(row=row_num, column=10).value = info['sample_message']
